...
class Exploratory_Randomized_Selfishucb(Algorithm):
    """
    ...
    def __str__(self):
        return "ExploratoryRandomizedSelfishucb"
    ...
    def run(self):
        ...
        while self.t < self.env.T:
            ...
            if self.randomized:
                ...
                #ucb_idx = (1/self.t)*ucb_idx + (1 - 1/self.t)*np.random.normal(0, 1/(self.t+1), size=(self.env.M, self.env.K))
                
            tt = max(self.t, 1)

            arms_t = 0

            if random.random() < 1/tt:
                arms_t = random_arm(self.env.M, self.env.K)
            else:
  ...
  #for player in range(M):
   ...
   #         # Explore: Choose a random arm with probability (1 - epsilon)
   #         selected_arm = np.random.choice(K)
   #     else:
   #         # Exploit: Choose the best arm with probability epsilon using the selfish KL-UCB metric function
